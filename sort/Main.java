...
public class Main{
	public static void main(String[] args){
		int [][]unsortedArr = {new int[10], new int[25], new int [50],
    		new int[75], new int[100], new int[150], new int[500], new int[1000]};
    	...
    	System.out.println("Nr  Size    Quick           Insertion       SquareRoot      QuickRecursion  Selection");
    	for (int i = 0; i < unsortedArr.length; i++){
			...
			sortedArr[i] = selectionSort.sort(unsortedArr[i].clone());
			System.out.printf("% d. %-6d  %-11.9fsec  %-11.9fsec  %-11.9fsec  %-11.9fsec  %-11.9fsec\n",
			(i+1), sortedArr[i].length, quickSort.time, insertionSort.time, squareRootSort.time, quicksortR.time, selectionSort.time);
		}
	}
